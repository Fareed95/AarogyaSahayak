import 'dart:convert';
import 'package:client/screens/YogaPoseAnalysisScreen.dart';
...
import 'YogaPoseAnalysisScreen.dart'; // Import your yoga pose analysis screen
...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  Future<void> _handleFileUpload() async {
    try {
      ...
      if (result == null) {
        AwesomeSnackbar.error(context, "Cancelled", "No file selected");
        return;
      }
      ...
      if (title == null || title.isEmpty) {
        AwesomeSnackbar.error(context, "Missing", "Title is required");
        return;
      }
      ...
      if (token == null) {
        AwesomeSnackbar.error(context, "Error", "Not logged in. Please login again.");
        return;
      }
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        AwesomeSnackbar.success(context, "Success", "Document uploaded successfully!");
      } else {
        final responseBody = await response.stream.bytesToString();
        AwesomeSnackbar.error(context, "Upload Failed", responseBody);
      }
    } catch (e) {
      print("Upload error: $e");
      AwesomeSnackbar.error(context, "Error", "Something went wrong while uploading.");
    }
  }
                      ...
                      IconButton(
                        ...
                        onPressed: () {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text("Use my location: (stub) — add geolocation integration."),
                            ),
                          );
                        },
                                  ...
                                  children: [
                                    IconButton(
                                      ...
                                      onPressed: () {
                                        ...
                                        ScaffoldMessenger.of(context).showSnackBar(
                                          SnackBar(
                                            content: Text("Call ${w['phone']} (stub)"),
                                          ),
                                        );
                                      },
                                    ),
                                    IconButton(
                                      ...
                                      onPressed: () {
                                        ...
                                        ScaffoldMessenger.of(context).showSnackBar(
                                          SnackBar(
                                            content: Text("Open map for ${w['name']} (stub)"),
                                          ),
                                        );
                                      },
                                    ),
                                        ...
                                        actions: [
                                          ...
                                          ElevatedButton(
                                            onPressed: () {
                                              ...
                                              ScaffoldMessenger.of(context).showSnackBar(
                                                SnackBar(
                                                  content: Text("Request sent to ${w['name']} (stub)"),
                                                ),
                                              );
                                            },
  ...
  // New Yoga Pose Analysis Section
  Widget _buildYogaPoseAnalysisSection(bool isDark) {
    return InkWell(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => const YogaPoseAnalysisScreen()),
        );
      },
      borderRadius: BorderRadius.circular(20),
      child: Container(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(
            colors: [
              isDark ? const Color(0xFF1A237E).withOpacity(0.3) : Colors.white,
              isDark ? const Color(0xFF1A237E).withOpacity(0.1) : const Color(0xFFE8EAF6),
            ],
            begin: Alignment.topLeft,
            end: Alignment.bottomRight,
          ),
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: isDark ? Colors.white.withOpacity(0.03) : const Color(0xFFC5CAE9),
          ),
        ),
        child: Row(
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 6),
                  Text(
                    "Get real-time feedback on your yoga poses using OpenCV and AI technology.",
                    style: TextStyle(
                      fontSize: 13,
                      color: isDark ? Colors.white.withOpacity(0.8) : Colors.grey[700],
                    ),
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        // Yoga Pose Analysis Section
                        _buildYogaPoseAnalysisSection(isDark),
                        const SizedBox(height: 32),

                        // Upload Section
                        _buildUploadSection(isDark, context),
                        const SizedBox(height: 32),
  ...
  Widget _buildUploadSection(bool isDark, BuildContext context) {
    return Column(
      children: [
        InkWell(
        ...
        InkWell(
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => const ReportInstancesScreen()),
            );
          },
          borderRadius: BorderRadius.circular(25),
          child: Container(
        ...
        children: [
          ...
          Text(
            ...
            style: TextStyle(
              fontSize: 20,
              fontWeight: FontWeight.bold,
              color: isDark ? const Color(0xFFFCA311) : const Color(0xFF14213D),
            ),
          ),
          const SizedBox(height: 12),
          Text(
